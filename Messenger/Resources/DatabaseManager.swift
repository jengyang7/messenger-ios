        ...
        database.child("\(conversation)/messages").observeSingleEvent(of: .value, with: { [weak self] snapshot in
            ...
            strongSelf.database.child("\(conversation)/messages").setValue(currentMessages) { (error, _) in
                ...
                strongSelf.database.child("\(currentEmail)/conversations").observeSingleEvent(of: .value, with: { snapshot in
                    ...
                    var targetConversations: [String: Any]?
                    
                    var position = 0
                    
                    for conversationDictionary in currentUserConversations  {
                        if let currentId = conversationDictionary["id"] as? String, currentId == conversation {
                            targetConversations = conversationDictionary
                            break
                        }
                        position += 1
                    }
                    targetConversations?["latest_message"] = updatedValue
                    guard let finalConversations = targetConversations else {
                        completion(false)
                        return
                    }
                    currentUserConversations[position] = finalConversations
                    strongSelf.database.child("\(currentEmail)/conversations").setValue(currentUserConversations, withCompletionBlock: { error, _ in
                        ...
                        strongSelf.database.child("\(otherUserEmail)/conversations").observeSingleEvent(of: .value, with: { snapshot in
                            ...
                            otherUserConversations[position] = finalConversations
                ...
                if var conversations = snapshot.value as? [[String: Any]] {
                    ...
                    for conversation in conversations {
                        if let id = conversation["id"] as? String,
                           id == conversationId {
                            print("found conversation to remove")
                            break
                        }
                        positionToRemove += 1
                    }
